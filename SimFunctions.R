...
# For more info please contact: subodh.selukar@stjude.org

### 2022-03-16: changed simRun to also include immunes
### 2022-03-16: added extra function for generating data and recording MZ 1996 test of immunes
...
immuneTest <- function(dat){
  ...
  pHat <- 1-summary(kmfit,times=lastObs,extend=TRUE)$surv # extend needed for some tail problems with log-logistic: weird behavior with lots of decimals and very long tail
  pCens <- 1-mean(dat$D) # proportion of censored
  
  return(c(pHat,pCens,lastObs,isCens))
}

rvTest <- function(H0, H1) {
  dev <- 2 * (H1 - H0)
  pval <- 0.5 - 0.5 * pchisq(dev,1)
}
...
simRun <- function(simN,numSims,simSeed,simParams,simDist,simTau,simAccrual){
  ...
  simOut <- matrix(nrow=numSims,
                   ncol=2*9+2*12+3+2+2+4) # each dist (*2 for cured and uncured) has ((numDistParams+1)*(1+2)+2) + 3 for M-Z,Shen,qN+2 for diagnostic event rate and is the last observation an event+4 for testing immunes
  ...
  for (i in 1:numSims){
    ...
    simOut[i,4:12] <- ratioTest(df,simTau,"exp")
    simOut[i,13:21] <- ratioTest(df,simTau,"expUnc")
    
    simOut[i,22:33] <- ratioTest(df,simTau,"wei")
    simOut[i,34:45] <- ratioTest(df,simTau,"weiUnc")
    
    simOut[i,46] <- rvTest(simOut[i,20], simOut[i,11])
    simOut[i,47] <- rvTest(simOut[i,44], simOut[i,32])
    ...
    simOut[i,48:49] <- c(mean(df$D==1),df$D[which.max(df$Y)]==1)
    
    simOut[i,50:53] <- immuneTest(df)
  }
...
simRunExtra <- function(simN,numSims,simSeed,simParams,simDist,simTau,simAccrual){
  ...
  colnames(simOut) <- c("MZ","Shen","qn","EventRate","LastObs")
...
simRunImmunes <- function(simN,numSims,simSeed,simParams,simDist,simTau,simAccrual){
  alldat <- simData(n=simN,sims=numSims,seed=simSeed,params=simParams,dist=simDist,tau=simTau,accrualTime=simAccrual)
  
  simOut <- matrix(nrow=numSims,
                   ncol=4) # 4 from test for immunes
  colnames(simOut) <- c("pHat","pCens","lastObs","isCens") # first column is pHat (1-KM(last observation time)), second is proportion censored, third is last obs time, fourth is whether last obs time is censored
  
  for (i in 1:numSims){
    df <- alldat[(simN*(i-1)+1):(i*simN),]
    
    simOut[i,] <- immuneTest(df)
  }
